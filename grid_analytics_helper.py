...
def create_congestion_risk(df):
    """Generates a feature to capture potential congestion risk by understanding moments where low supply of electricity coincide with significant amount of unexpected outages on the grid.
    ...
    Returns:
       pd.DataFrame: A dataframe containing the congestion risk feature.
    """
    req_features = [
    ...
    if all(col in df.columns for col in req_features):
        df["congestion_risk_by_region"] = (
            ((df["forced_outage_pct_by_region"] / 100) * (df["outage_intensity"] / 100))
            * 100
        ).fillna(0)
        df["congestion_risk_PJM"] = (
            ((df["forced_outage_pct_PJM"] / 100) * (df["outage_intensity"] / 100)) * 100
        ).fillna(0)
        return df
    else:
        raise ValueError(
            "'forced_outage_pct_by_region', 'forced_outage_pct_PJM', or 'outage_intensity' are not in the dataframe."
        )
    ...
    def update_calendar_heatmap(change=None):
        ...
        if not filtered_data.empty:
            ...
            fig = px.imshow(
                ...
                labels={
                    "color": f"{feature.replace('_', ' ').title()} {feature_units}"
                },
