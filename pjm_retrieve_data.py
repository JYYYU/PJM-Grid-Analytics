...
class PJMMonthlyFTRBidsFetcher(PJMHeaderSetUp):
    ...
    def __init__(
        ...
        Args:
          ...
          If provided, only data for this market name will be fetched.
            start_datetime(tuple of int, optional): With the format (YEAR, MONTH). Defaults to None.
            end_datetime(tuple of int, optional): With the format (YEAR, MONTH). Defaults to None.
    ...
    def fetch_all_monthly_ftr_bids_for_market(self, market_name, **kwargs):
        ...
        # Process any quarterly FTRs
        quarter_to_month = {"Q1": "MAR", "Q2": "JUN", "Q3": "SEP", "Q4": "DEC"}
        all_data_df["is_quarterly_ftr"] = all_data_df["period_type"].isin(
            quarter_to_month
        )  # Flag quarterly FTRs
        all_data_df["period_type"] = all_data_df["period_type"].replace(
            quarter_to_month
        )  # Convert quarterly FTRs period type to the last month of their respective quarter
